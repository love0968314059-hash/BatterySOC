...
class ImprovedAISOCEstimator:
    ...
    def predict_batch(self, voltage, current, time, temperature):
        ...
        # 前sequence_length个点使用初始SOC + AH积分
        dt = np.diff(time)
        dt = np.concatenate([[1.0], dt])
        dt = np.clip(dt, 0.1, 100.0)
        
        soc = self.initial_soc
        for i in range(min(self.sequence_length, n)):
            soc_estimated[i] = soc
            if i < n - 1:
                soc = soc + current[i] * dt[i] / 3600 / self.nominal_capacity * 100
        ...
        with torch.no_grad():
            for i in range(self.sequence_length, n):
                ...
                soc_pred = self.model(seq_tensor).cpu().numpy()[0, 0]
                soc_estimated[i] = soc_pred * 100  # 转换回0-100%
